      ...
      >
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          <Tabs value={value} onChange={handleChange}>
            <Tab label="ERP登入" sx={{ width: 1 / 2 }} value={1} />
            <Tab label="ERP註冊" sx={{ width: 1 / 2 }} value={2} />
          </Tabs>
        </Box>
...
function LoginForm({ value, ...props }: { value: number }): JSX.Element {
...
function RegisterForm({ value, ...props }: { value: number }): JSX.Element {
  const navigate = useNavigate();
  const {
    handleSubmit,
    control,
